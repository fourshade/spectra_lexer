...
class StenoLexer(SpectraComponent):
    ...
    _keys: LexerKeys                   # Current parsed keys, used in default return rule if none others are valid.
    ...
    def _build_best_rule(self, pairs:List[Tuple[str,str]]) -> StenoRule:
        ...
        for keys, word in pairs:
            ...
            built_maps += self._generate_maps(LexerKeys.cleanse_from_rtfcre(keys), word)
    ...
    def _generate_maps(self, keys:LexerKeys, word:str) -> List[LexerResult]:
        ...
        while stack:
            ...
            if test_keys:
                test_keys = self._check_special(test_keys, rulemap, wordptr)
            ...
            if test_keys:
                ...
                for r in get_rule_matches(test_keys, test_word, is_full_stroke, is_full_word):
                ...
                maps.append(LexerResult(keys, word, rulemap))
                best_letters = max(best_letters, lc)
        return maps

    def _check_special(self, test_keys:LexerKeys, rulemap:list, wordptr:int) -> LexerKeys:
        ...
        if test_keys.is_star(0) and (len(test_keys) == 1 or test_keys.is_separator(1)):
            ...
            return test_keys.without(rulemap[-1].rule.keys)
        ...
        if test_keys.is_separator(0):
            ...
            return test_keys.without(_RULE_SEP.keys)
        return test_keys

    def _decipher_star(self, test_keys:LexerKeys, rulemap:list) -> str:
