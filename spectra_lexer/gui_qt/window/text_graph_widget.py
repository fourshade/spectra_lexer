...
class TextGraphWidget(QTextEdit):
    ...
    def mouseMoveEvent(self, event:QMouseEvent) -> None:
        ...
        row, col = self._cursor_pos(event.pos())
        # Don't waste time if the row and column haven't changed.
    ...
    def mousePressEvent(self, event:QMouseEvent) -> None:
        """ A mouse click always sends an event for the current character position. """
        if not self._mouse_enabled:
            return super().mousePressEvent(event)
        self.textMouseAction.emit(*self._cursor_pos(event.pos()), True)

    def _cursor_pos(self, local_pos:QPoint) -> tuple:
        """ Return the row and column of the mouse's position within the text. """
        ...
        return location_cursor.blockNumber(), location_cursor.columnNumber()
