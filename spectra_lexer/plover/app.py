...
class PloverPluginApplication(GUIQtBaseApplication):
    """ Main application class for Plover plugin. It is the only class that should directly access Plover's objects.
        Receives and processes dictionaries and translations from Plover using callbacks.
        Since Plover initiates all interactions, all engine commands should be async. """
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        if not _compatibility_check():
            self.engine_call_async("set_status_message", _INCOMPATIBLE_MESSAGE)
            return
    ...
    def new_window(self, window:BaseWindow) -> None:
        ...
        self.engine_call_async("gui_menu_set_visible", False)
    ...
    def load_dict_collection(self, steno_dc:PloverStenoDictCollection) -> None:
        ...
        if steno_dc and steno_dc.dicts:
            ...
            self.engine_call_async("new_search_dict", parsed)
    ...
    def on_new_translation(self, _, new:Sequence[PloverAction]) -> None:
        ...
        if new and t_text and any(map(str.isalnum, t_text)):
            ...
            self.engine_call_async("app_query_and_display", join_strokes(new_strokes), "".join(new_text))
