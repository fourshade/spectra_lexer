...
class ArrowPathGenerator:
    ...
    def __init__(self) -> None:
        self.endpoint_shift = 12.0      # Amount to pull the arrow endpoints back away from the element centers.
        self.curve_shift = 15.0         # Amount to "bend" the arrow body outward perpendicular from the straight case.
        self.head_length_ratio = 0.033  # Extra length for head edges per unit of overall arrow length.
        self.min_head_length = 5.0      # Minimum length for head edges (the lines that make the "point" of the arrow).
        self.spread_angle = pi / 8      # Angle in radians between each head edge and the arrow body.

    def connect(self, tail:complex, head:complex) -> str:
        """ Return an SVG path string for a curved arrow path from <tail> to <head>.
...
class ChainPathGenerator:
    ...
    def connect(self, start:complex, end:complex) -> str:
        """ Return an SVG path string for a complete chain path from <start> to <end>.
            If layers are required, use iter_halves instead and add layers for each half separately. """
        return "".join(self.iter_halves(start, end))

    def iter_halves(self, start:complex, end:complex) -> Iterator[str]:
        """ Yield SVG path strings for alternating halves of a chain path from <start> to <end>. """
        vec = end - start
        length = abs(vec)
        unit_vec = vec / length
        origins = [start + unit_vec * offset for offset in self._link_offsets(length)]
        for starts_clockwise in (False, True):
            yield self._draw_hemilinks(origins, unit_vec, starts_clockwise)
    ...
    def _draw_hemilinks(self, origins:List[complex], unit_vec:complex, is_cw:bool) -> str:
        """ Return a path string for alternating halves of a chain link starting at every position in <origins>.
