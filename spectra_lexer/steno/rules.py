...
UNMATCHED_FLAG_SET = RuleFlags({RuleFlags.UNMATCHED})
GENERATED_FLAG_SET = RuleFlags({RuleFlags.GENERATED})
...
class StenoRule(NamedTuple):
    ...
    @classmethod
    def unmatched(cls, keys:str):
        """ Make a special rule with unmatched keys. """
        return cls(keys, "", UNMATCHED_FLAG_SET, "unmatched keys", ())
